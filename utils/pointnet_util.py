...
def sample_and_group(npoint,
    ...
    new_xyz = sample.gather_point(xyz, sample.farthest_point_sample(
        npoint, xyz))  # (batch_size, npoint, 3)
    if knn:
        _, idx = group.knn_point(nsample, xyz, new_xyz)
    else:
        idx, pts_cnt = group.query_ball_point(radius, nsample, xyz, new_xyz)
    grouped_xyz = group.group_point(xyz,
                                    idx)  # (batch_size, npoint, nsample, 3)
    grouped_xyz -= tf.tile(tf.expand_dims(new_xyz, 2),
                           [1, 1, nsample, 1])  # translation normalization
    if points is not None:
        grouped_points = group.group_point(
...
def sample_and_group_all(xyz, points, use_xyz=True):
    ...
    new_xyz = tf.constant(np.tile(
        np.array([0, 0, 0]).reshape((1, 1, 3)), (batch_size, 1, 1)),
...
def pointnet_sa_module_msg(xyz,
    ...
    data_format = 'NCHW' if use_nchw else 'NHWC'
    with tf.variable_scope(scope) as sc:
        new_xyz = sample.gather_point(xyz,
                                      sample.farthest_point_sample(npoint, xyz))
        new_points_list = []
        for i in range(len(radius_list)):
            radius = radius_list[i]
            nsample = nsample_list[i]
            idx, pts_cnt = group.query_ball_point(radius, nsample, xyz, new_xyz)
            grouped_xyz = group.group_point(xyz, idx)
            grouped_xyz -= tf.tile(tf.expand_dims(new_xyz, 2),
                                   [1, 1, nsample, 1])
            if points is not None:
                grouped_points = group.group_point(points, idx)
                if use_xyz:
...
def pointnet_fp_module(xyz1,
    ...
    '''
    with tf.variable_scope(scope) as sc:
        dist, idx = interpolate.three_nn(xyz1, xyz2)
        dist = tf.maximum(dist, 1e-10)
        norm = tf.reduce_sum((1.0 / dist), axis=2, keep_dims=True)
        norm = tf.tile(norm, [1, 1, 3])
        weight = (1.0 / dist) / norm
        interpolated_points = interpolate.three_interpolate(
            points2, idx, weight)

        if points1 is not None:
            new_points1 = tf.concat(axis=2,
                                    values=[interpolated_points, points1
